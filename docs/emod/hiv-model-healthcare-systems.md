...
Intervention campaigns in the EMOD HIV model typically involve health care, treatment, and
...
Health care in EMOD is bi-directional: it can be applied to individuals, or it can sought by
...
Individuals can be enrolled in treatment due to the outcomes of diagnostic testing, but EMOD
...
For individuals making a random choice, EMOD will base the decision on a random coin flip, or if
...
Over the past several decades, such treatment guidelines have changed, and EMOD allows for these
...
The below *JSON (JavaScript Object Notation)* example shows the **HIVARTStagingCD4AgnosticDiagnostic** intervention class,
...
*See example: [hiv-cd4agnosticdiagnostic.json](../json/hiv-cd4agnosticdiagnostic.json)*
...
ART. The [Swiss HIV Cohort Study][swiss-hiv-cohort-study] demonstrated that absolute changes in CD4
...
another reconstitution of CD4. EMOD calculates survival time on ART discontinuation by first
...
![An example of CD4 counts and prognosis for a patient on and off ART. Art by Samantha Zimmerman.](../images/hiv/intrahost.png)
